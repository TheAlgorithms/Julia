...
function gradient_descent(W::Matrix, b, X::Matrix, Y::Matrix, alpha::Float64, num_iters::Int64)
    m = length(Y); 
    J_history = [];
    for i = 1:num_iters
...
function linear_regression(X::Matrix, Y::Matrix, alpha::Float64, num_iters::Int64)
    W = zeros(1, size(X, 1));
    b = 0;
