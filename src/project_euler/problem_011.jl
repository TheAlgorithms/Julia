...
function problem_011()
    ...
    rowsize, colsize = size(grid)
    max_prod = 1
    for idx in eachindex(IndexCartesian(), grid)
        x, y = idx[1], idx[2]
        # horizontal maximum
        if x + 4 <= rowsize
            max_prod = max(max_prod, dir_prod(grid, x, y, 1, 0))
        end
        # vertical maximum
        if y + 4 <= colsize
            max_prod = max(max_prod, dir_prod(grid, x, y, 0, 1))
        end
        # leading diagonal direction maximum
        if x + 4 <= rowsize && y + 4 <= colsize
            max_prod = max(max_prod, dir_prod(grid, x, y, 1, 1))
        end
        # off diagonal direction maximum
        if x + 4 <= rowsize && y - 4 >= 1
            max_prod = max(max_prod, dir_prod(grid, x, y, 1, -1))
        end
    end
